...
import { 
  ...
  ExternalLink, Link2, BadgeCheck, ChevronLeft
} from "lucide-react";
...
export default function SellerList({ products, onRefreshProducts, initialSearchQuery, onViewProduct }: SellerListProps) {
  ...
  return (
    <div id="seller-list-view" className="max-w-7xl mx-auto px-4 py-8">
      ...
      {/* Seller's Full Shop Page — replaces the list view entirely (not an
          overlay) so the seller gets a real, spacious, dedicated storefront
          rather than a cramped popup. */}
      {activeSellerModal && (
        <div id="seller-shop-page" className="pb-28 lg:pb-10">

          {/* Back to directory */}
          <button
            onClick={handleCloseSellerModal}
            className="flex items-center gap-1.5 text-slate-500 hover:text-slate-800 font-semibold text-xs mb-5 cursor-pointer transition-colors"
          >
            <ChevronLeft className="w-4 h-4" />
            {language === "EN" ? "Back to Sellers Directory" : "Kembali ke Direktori Penjual"}
          </button>

          {/* Hero cover banner */}
          <div className="relative rounded-3xl overflow-hidden bg-gradient-to-br from-slate-900 via-slate-800 to-emerald-950 text-white shadow-xl">
            <div className="absolute inset-0 weave-texture opacity-[0.06] pointer-events-none" aria-hidden="true"></div>
            ...
            <div className="relative p-7 sm:p-10">
              <div className="flex flex-col sm:flex-row items-start sm:items-center gap-6">
                <div className="w-24 h-24 sm:w-28 sm:h-28 rounded-3xl overflow-hidden border-2 border-emerald-500 bg-white/10 shrink-0 flex items-center justify-center shadow-lg">
                ...
                <div className="min-w-0">
                  <div className="flex flex-wrap gap-2 items-center">
                    <span className="bg-emerald-600/90 text-white text-[10px] font-bold px-2.5 py-1 rounded-md uppercase tracking-wider">
                      {activeSellerModal.category}
                    </span>
                    {activeSellerModal.verificationTier === "Gold" ? (
                      <span className="flex items-center gap-1 text-[11px] font-extrabold bg-emerald-950/40 px-2.5 py-1 rounded-md border border-[#0f9d58]/30" style={{ color: "#3ddc84" }}>
                        <ShieldCheck className="w-3.5 h-3.5" style={{ color: "#3ddc84" }} />
                        {language === "EN" ? "Gold Verified Seller" : "Penjual Disahkan Emas"}
                      </span>
                    ) : activeSellerModal.verificationTier === "Silver" ? (
                      <span className="flex items-center gap-1 text-[11px] font-extrabold bg-slate-900/40 px-2.5 py-1 rounded-md border border-[#c0c0c0]/30" style={{ color: "#c0c0c0" }}>
                        <ShieldCheck className="w-3.5 h-3.5" style={{ color: "#c0c0c0" }} />
                        {language === "EN" ? "Silver Verified Seller" : "Penjual Disahkan Perak"}
                      </span>
                    ) : activeSellerModal.verificationTier === "Bronze" ? (
                      <span className="flex items-center gap-1 text-[11px] font-bold bg-orange-950/40 px-2.5 py-1 rounded-md border border-[#cd7f32]/30" style={{ color: "#cd7f32" }}>
                      ...
                      <span className="flex items-center gap-1 text-emerald-400 text-[11px] font-bold bg-emerald-950/50 px-2.5 py-1 rounded-md border border-emerald-500/30">
                        <ShieldCheck className="w-3.5 h-3.5 text-emerald-400" />
                        {language === "EN" ? "Licensed / SSM Verified Business" : "Perniagaan Berlesen / SSM Disahkan"}
                      </span>
                    ) : (
                      <span className="flex items-center gap-1 text-amber-400 text-[11px] font-bold bg-amber-950/50 px-2.5 py-1 rounded-md border border-amber-500/10">
                  ...
                  <div className="flex items-center gap-2 mt-3">
                    <h1 className="font-extrabold text-3xl sm:text-4xl leading-tight truncate">
                      {activeSellerModal.businessName}
                    </h1>
                    <button
                      type="button"
                      onClick={() => handleShareSellerProfile(activeSellerModal.id, activeSellerModal.businessName)}
                      className="p-2 rounded-xl bg-white/10 hover:bg-white/20 text-white transition-all cursor-pointer shadow-sm flex items-center justify-center shrink-0"
                  ...
                  <div className="flex flex-wrap items-center gap-x-5 gap-y-1.5 mt-4 text-xs">
                    ...
                    <span className="flex items-center gap-1.5 text-slate-300">
                      ...
                      {shopProducts.length} {language === "EN" ? "Products" : "Produk"}
                    </span>
                    <span className="flex items-center gap-1.5 text-slate-300">
                      <Calendar className="w-4 h-4 text-slate-400" />
                      {language === "EN" ? "Since " : "Sejak "}{activeSellerModal.establishedYear || "—"}
                    </span>
                  </div>
                  ...
                  {activeSellerModal.latestUpdate && activeSellerModal.latestUpdateAt && (Date.now() - new Date(activeSellerModal.latestUpdateAt).getTime()) < 3 * 24 * 60 * 60 * 1000 && (
                    <div className="flex items-start gap-1.5 bg-amber-400/10 border border-amber-300/20 rounded-lg px-3 py-2 mt-4 max-w-lg">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mt-8">
            ...
            <div className="lg:col-span-2 space-y-8 min-w-0">
              ...
              {/* About / Entrepreneur Story */}
              <div className="space-y-2.5">
                <h4 className="text-[11px] font-bold text-amber-600 bg-amber-50 px-2.5 py-1 rounded-md w-fit uppercase tracking-wider">
                  {language === "EN" ? "Entrepreneur Story & Goal" : "Kisah & Matlamat Usahawan"}
                </h4>
                <div className="bg-gradient-to-br from-amber-50/40 to-orange-50/25 p-5 rounded-2xl border border-amber-100/30">
                  <p className="text-slate-700 text-sm italic leading-relaxed font-serif">
            ...
            {/* Sticky sidebar: contact card + shop overview + desktop CTA */}
              ...
              <div className="lg:sticky lg:top-24 space-y-4">
                ...
                <div className="bg-white border border-slate-100 rounded-2xl shadow-sm p-5 space-y-4">
                  ...
                  <a
                    ...
                    rel="noopener noreferrer"
                    onClick={() => handleTrackContactClick(activeSellerModal.id)}
                    className="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-3 px-4 rounded-xl shadow transition-all flex items-center justify-center gap-1.5 text-xs cursor-pointer"
                  >
          ...
          <div className="lg:hidden fixed bottom-0 inset-x-0 z-40 p-3 bg-white/95 backdrop-blur border-t border-slate-100 shadow-[0_-4px_16px_rgba(0,0,0,0.06)] flex gap-2.5">
            <button
              onClick={handleCloseSellerModal}
              className="px-4 py-3 rounded-xl border border-slate-200 hover:bg-slate-100 text-slate-700 font-bold text-xs cursor-pointer text-center shrink-0"
            >
            ...
            <a
              ...
              rel="noopener noreferrer"
              onClick={() => handleTrackContactClick(activeSellerModal.id)}
              className="flex-grow bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-3 px-4 rounded-xl shadow transition-all flex items-center justify-center gap-1.5 text-xs cursor-pointer"
            >
              <Phone className="w-3.5 h-3.5" />
              {language === "EN" ? "Chat on WhatsApp" : "Sembang di WhatsApp"}
            </a>
